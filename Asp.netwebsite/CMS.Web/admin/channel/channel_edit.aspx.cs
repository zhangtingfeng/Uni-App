    ...
    public partial class channel_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            else
            {
                #region 添加默认URl配置
                //首页url
                Model.url_rewrite urlModelIndex = new Model.url_rewrite();
                urlModelIndex.name = model.name;
                urlModelIndex.type = "index";
                urlModelIndex.page = model.name + ".aspx";
                urlModelIndex.inherit = GetInherit(urlModelIndex.type);
                urlModelIndex.templet = model.name + ".html";
                urlModelIndex.channel = model.name;
                List<Model.url_rewrite_item> indexItems = new List<Model.url_rewrite_item>
                    {
                ...
                urlModelIndex.url_rewrite_items = indexItems;
                urlBll.Add(urlModelIndex);
                //列表url
                Model.url_rewrite urlModelList = new Model.url_rewrite();
                urlModelList.name = model.name + "_list";
                urlModelList.type = "list";
                urlModelList.page = model.name + "_list.aspx";
                urlModelList.inherit = GetInherit(urlModelList.type);
                urlModelList.templet = model.name + "_list.html";
                ...
                List<Model.url_rewrite_item> listItems = new List<Model.url_rewrite_item>
                    {
                        new Model.url_rewrite_item(){
                            path=model.name+"/list.aspx",
                            pattern=model.name+"/list.aspx"
                        },
                        new Model.url_rewrite_item(){
                            path=model.name+"/list-{0}.aspx",
                            pattern=model.name+"/list-(\\d+).aspx",
                            querystring="category_id=$1"
                        },
                        new Model.url_rewrite_item(){
                            path=model.name+"/list-{0}-{1}.aspx",
                            pattern=model.name+"/list-(\\d+)-(\\w+).aspx",
