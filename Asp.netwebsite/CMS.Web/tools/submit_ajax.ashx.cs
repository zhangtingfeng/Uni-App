...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "user_register": //用户注册
                    ...
                    break;
                case "user_verify_token": //获取发送短信令牌(防注册机)
                    user_verify_token(context);
                    break;
        ...
        #region 获取发送手机短信令牌OK=========================
        private void user_verify_token(HttpContext context)
        {
            //判断是否在规定间隔内才能申请令牌
            if (context.Session[DTKeys.SESSION_SMS_TOKEN_TIME] != null)
            {
                //将时间值进行与当前时间对比,如超过有效期则分匹配令牌
                DateTime tokenTime = DateTime.Parse(context.Session[DTKeys.SESSION_SMS_TOKEN_TIME].ToString());
                TimeSpan ts = DateTime.Now - tokenTime;
                if (ts.Minutes < userConfig.regsmsexpired)
                {
                    context.Response.Write("{\"status\":0, \"msg\":\"申请失败，请勿重复申请通讯令牌！\"}");
                    return;
                }
            }
            //如未申请过或超过有效期则分配令牌
            string token = Utils.GetCheckCode(16);
            context.Session[DTKeys.SESSION_SMS_TOKEN] = token;
            context.Session[DTKeys.SESSION_SMS_TOKEN_TIME] = DateTime.Now;
            context.Response.Write("{\"status\":1, \"token\":\"" + token + "\", \"msg\":\"成功申请通讯令牌！\"}");
            return;
        }
        #endregion
        ...
        {
            string token = Utils.ToHtml(DTRequest.GetString("token"));
            ...
            //检查令牌是否有效
            if (context.Session[DTKeys.SESSION_SMS_TOKEN] != null && context.Session[DTKeys.SESSION_SMS_TOKEN].ToString() == token)
            {
                ...
                //发送成功后作废令牌
                context.Session[DTKeys.SESSION_SMS_TOKEN] = null;
                //通知成功
                context.Response.Write("{\"status\":1, \"time\":\"" + userConfig.regsmsexpired + "\", \"msg\":\"手机验证码发送成功！\"}");
                return;
            }
            //令牌无效返回失败信息
            context.Response.Write("{\"status\":0, \"msg\":\"发送失败，" + userConfig.regsmsexpired + "分钟内请勿重复发送！\"}");
            return;
        }
        #endregion
        ...
        private void user_amount_recharge(HttpContext context)
        {
            ...
            if (!new BLL.site_payment().Exists(payment_id))
            {
