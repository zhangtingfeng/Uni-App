...
export function mkdirpp(dir: string, mode?: number): Promise<string> {
    return new Promise<string>((resolve, reject) => {

        mkdirp(dir, mode || 0o755, (err: any, made: any) => {
            if (err) {
                reject(err);
            } else {
                resolve(made);
...
export function readFile(target: string): Promise<string> {
    return new Promise((resolve, reject) => {
        fsExtra.readFile(target, '', (err, data) => {
            if (err) {
                return reject(err);
            }
            resolve(data);
...
export function exec(
    ...
    return new Promise((resolve, reject) => {
        console.log(base(command) + ' ' + args.join(' '));
        cp.exec(base(command) + ' ' + args.join(' '), { maxBuffer: 1024 * 4000 }, (err, stdout) => {
            if (err) {
                console.log('\n\rError: ' + err);
                return reject(err);
            }

            console.log('\n\rSuccess: ' + stdout.toString());
...
export function fromDist(modulename: string) {
    return baseDir(`./dist/${modulename}`);
}

export function getPackageFilePath(pkg: string, filename: string) {
    return baseDir(`./packages/${pkg}/${filename}`);
}
