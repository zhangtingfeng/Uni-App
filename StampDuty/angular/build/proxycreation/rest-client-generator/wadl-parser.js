    ...
    return util.fetchFile(this.options.input)
        .then(function (wadlContent) {
            ...
            return Promise.all(
                ...
                .then(function() {
                    this.select('//ns:application/ns:grammars/xs:schema', doc).map(function (xsdDoc) {
                        this.parseGrammarSchema(xsdDoc);
                    }.bind(this));
                }.bind(this))
                .then(function () {
                    var rootUrl = null;
...
WadlParser.prototype.parseInnerFieldType = function (elementNode) {
    var type = this.select('xs:complexType/xs:sequence/xs:element|xs:complexType/xs:complexContent/xs:extension/xs:sequence/xs:element', elementNode).map(function (elemNode) {
        return this.parseFieldElement(elemNode);
    }.bind(this));
    return type;
}
...
WadlParser.prototype.parseFieldElement = function (elementNode) {
    ...
    if (type.indexOf('xs:') === 0 || type.toUpperCase().indexOf('GUID') > 0) {
        type = getJsType(type);
    } else {
    ...
    if(type === '') {
        var responsetype = this.parseInnerFieldType(elementNode);
        if(responsetype[0] !== null && responsetype[0] !== undefined) {
            var customType = responsetype[0];
            type = customType.type;
            optional = customType.optional;
            array  = customType.array;
        }
    }

    return {
        name: util.format(elementNode.getAttribute('name'), this.options.propertynameCapitalize),
        type: type,
        optional: optional,
        array: array
    };
...
WadlParser.prototype.parseFieldAttribute = function (attributeNode) {
    ...
    return {
        name: util.format(attributeNode.getAttribute('name'), this.options.propertynameCapitalize),
        type: type,
        optional: optional,
        array: false
    };
...
function getJsType(type) {
    ...
    let typevalue = type.toUpperCase().indexOf('GUID') > 0 ? 'xs:GUID' : type;
...
WadlParser.prototype.readResource = function (resourceNode, parentUrlPath, parentUrlPathParams, parentName) {
    ...
    this.select('ns:method', resourceNode).forEach(function (methodNode) {
        var method = {
            name: util.format(methodNode.getAttribute('id') ? methodNode.getAttribute('id').replace(this.options.methodExclude, '') : (resourceNode.getAttribute('path') ? resourceNode.getAttribute('path').replace('/', '') : 'method' + this.methodCounter++), true),
        ...
        var requestResponseFunc = function (variant) {
            ...
            if (node) {
                ...
                if (representationNode) {
                    var typeNameinWadl = representationNode.getAttribute('element');
                    var type = this.types[typeNameinWadl];
                    if (type) {
                        method[variant + 'Type'] = type.name;
                    } 
                    else {
                        var typeFound = false;
                        for(var position in this.types) {
                            if(this.types[position].name !== undefined && this.types[position].name === typeNameinWadl) {
                                type = this.types[position];
                                method[variant + 'Type'] = type.name;
                                typeFound = true;
                                break;
                            }
                        ...
                        if(!typeFound) {
                            method[variant + 'Type'] = getJsType('any');
                        }
                    }
        ...
        var serviceName = util.format(name.replace(this.options.serviceExclude, ''), this.options.capitalize) + this.options.serviceSuffix;
    ...
    var serviceName = util.format(name.replace(this.options.serviceExclude, ''), this.options.capitalize) + this.options.serviceSuffix;
    // console.log('urlpath --> ' + urlPath + ', name -->' + name + 'servicemethod count --> ' + this.services[serviceName])
