...
function cloneObj<T extends Record<keyof T, any>>(a: T): T {
    const b: T = {} as T;
    const ownKeys = Reflect.ownKeys(a) as (keyof T)[];
    for (const k of ownKeys.filter(isString)) {
        const t = typeof a[k];
        b[k] = t === 'object' ?
