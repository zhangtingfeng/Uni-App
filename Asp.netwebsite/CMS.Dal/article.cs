...
namespace DTcms.DAL
{
	...
	public partial class article
	{
        ...
        public Model.article GetTop1Model(string channel_name,int? category_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 * from " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where category_id=@category_id  order by sort_id asc");
            SqlParameter[] parameters = {
        ...
        public Model.article GetPreModel(string channel_name, int category_id, int article_id)
        {
            var strSql = new StringBuilder();
            strSql.Append("select top 1 * from " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" WHERE category_id=@category_id and id >@id ORDER BY id ASC");
            return DapperHelper.Get<Model.article>(strSql.ToString(), 
                new { category_id = category_id, id = article_id 
            });
        }
        public Model.article GetNextModel(string channel_name, int category_id, int article_id)
        {
            var strSql = new StringBuilder();
            strSql.Append("select top 1 * from " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" WHERE category_id=@category_id and id <@id ORDER BY id DESC");
            return DapperHelper.Get<Model.article>(strSql.ToString(), new
            {
        ...
        public List<Model.article> GetListAsync(string channel_name, int Top, string strWhere, string filedOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (Top > 0)
            ...
            strSql.Append(" order by " + filedOrder);
            return DapperHelper.GetList<Model.article>(strSql.ToString());
        }
        /// <summary>
        /// 获得查询分页数据
        /// </summary>
        ...
        /// 获得查询分页数据
        /// </summary>
        public List<Model.article> GetListAsync(string channel_name, int category_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            if (category_id > 0)
            ...
            recordCount = Convert.ToInt32(DapperHelper.Get<int>(PagingHelper.CreateCountingSql(strSql.ToString())));
            return DapperHelper.GetList<Model.article>(PagingHelper.CreatePagingSql(recordCount, pageSize, pageIndex, strSql.ToString(), filedOrder));
        }
        #endregion
