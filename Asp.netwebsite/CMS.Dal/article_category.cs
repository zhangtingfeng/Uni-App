...
namespace DTcms.DAL
{
    ...
    public partial class article_category
    {
        ...
        public Model.article_category GetModel(string link_url)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.article_category model = new Model.article_category();
            ...
            strSql.Append("select top 1 " + str1.ToString().Trim(','));
            strSql.Append(" from " + databaseprefix + "article_category");
            strSql.Append(" where link_url=@link_url");
            SqlParameter[] parameters = {
                    new SqlParameter("@link_url", SqlDbType.NVarChar)};
            parameters[0].Value = link_url;
            DataTable dt = DbHelperSQL.Query(strSql.ToString(), parameters).Tables[0];
        ...
        /// <param name="channel_id">频道ID</param>
        /// <returns></returns>
        public List<Model.article_category> GetList(int parent_id, int? channel_id)
        {
            var strSql = new StringBuilder();
            strSql.Append("select * from " + databaseprefix + "article_category where parent_id=" + parent_id);
            if (channel_id.HasValue)
            {
                strSql.Append(" and channel_id=" + channel_id);
            }
            strSql.Append(" order by sort_id asc,id desc");
            return DapperHelper.GetList<Model.article_category>(strSql.ToString());
        }
        #endregion
    }
}